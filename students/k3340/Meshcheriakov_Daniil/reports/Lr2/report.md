...
Это позволяет мгновенно получить точный ответ для N = 10¹³.  
При этом цель задачи — не только получить сумму, но и сравнить подходы по производительности, поэтому дополнительно используется `N_BENCH = 100_000_000` с **реальной итерацией** через `range()`.

Таким образом:
- `N = 10^13` + формула — проверка корректности и декомпозиции задачи;
- `N_BENCH` + итерация — честное сравнение `threading` / `multiprocessing` / `asyncio` на CPU-bound нагрузке.
...
- Ключевой итог: в CPU-bound сценарии лучшим подходом является **multiprocessing**, а `threading`/`asyncio` (без процессов) нужны здесь скорее для демонстрации ограничений модели выполнения.
...
- Ключевой итог: в I/O-bound сценарии лучшую эффективность показывает **asyncio**, а multiprocessing в таких задачах обычно избыточен.
...
Лабораторная работа показывает, что вопрос «какой метод лучше» не имеет единственного ответа без указания типа нагрузки:
- для **CPU-bound** лучше `multiprocessing`;
- для **I/O-bound** лучше `asyncio` (или `threading` как более простой вариант);
- `threading` в CPU-задачах ограничен GIL, а `multiprocessing` в I/O-задачах страдает от лишнего overhead.

Именно поэтому в отчёте использованы **две разные задачи**: первая раскрывает влияние GIL и моделей параллелизма на вычислениях, вторая — реальную оптимизацию при работе с сетью и БД.
